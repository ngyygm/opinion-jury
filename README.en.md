...
> **TL;DR**: Opinion Jury is a public-opinion risk review system built on the Claude Code skill framework. It constructs multi-dimensional virtual characters who debate in multi-blind isolated "simulated courtrooms" across multiple rounds, ultimately producing traceable, auditable risk assessment reports — helping you discover risks you may never have thought of *before* you publish.
...
- [Role Design System](#role-design-system)
  ...
  - [14 Independent Behavioral Attributes](#14-independent-behavioral-attributes)
...
- [Using as a Claude Code Skill](#using-as-a-claude-code-skill)
...
Traditional content moderation can only tell you "this sentence is risky." **Opinion Jury tells you**: *a price-sensitive user who has long believed the brand overcharges, is active in gaming communities, loves screenshot-ranting, and has 100K followers* — how they would dismantle your content for their own interests, and *why they wouldn't succeed*.
...
| 🧠 **Think/Say Separation** | Each character has "private thoughts" (invisible) and "public speech" (visible), simulating the real-world gap between what people say and what they actually think |
...
| 📊 **Balanced Assessment** | Doesn't just report risks — objectively identifies content strengths, whether attackers gain the upper hand, and whether neutral observers are swayed |
...
The system uses 14 independent behavioral attributes to build characters. Correlations between attributes must have explicit written justification. Auto-assuming "extreme = dishonest" or "moderate = rational" is prohibited.
...
### Rule 8: Balanced Assessment — Risks and Strengths Together
...
The final report must objectively describe the simulated debate:

| What to report | Description |
|---------------|-------------|
| **Attack paths** | Who would attack, how, and why |
| **Attack effectiveness** | Whether neutral observers were swayed, whether attackers gained the upper hand |
| **Content strengths** | Which phrasings survived stress testing |
| **Real risks** | Which issues must be fixed |
| **Overreaction warning** | Flagging which "risks" likely won't cause real-world problems |
| **Overall verdict** | Whether the content is already good enough as-is |
...
│     14 independent attributes (see below)          │
├──────────────────────────────────────────────────┤
...
### 14 Independent Behavioral Attributes

Each character's behavior is described by 14 independent dimensions with explicit enum values:
...
- All speech is appended to the public transcript file, readable by all debaters in subsequent rounds
...
The final report is not just a "risk checklist" — it's a **balanced evaluation**:

### Required Content

| Dimension | Report Chapter | Description |
|-----------|---------------|-------------|
| Who attacks | Attack Path Analysis | What kind of person, under what circumstances, how they attack |
| Who gets distorted | Misrepresentation Risk | Whose words get taken out of context, which side gets weaponized |
| Who are victims | Victim Identification | Whose interests are harmed, who are collateral victims |
| Interest damage | Interest Damage Map | 12 stakeholder categories' gains/losses, public/private gaps, strategic behavior |
| Response plans | Contingency Matrix | Plans for 7 scenarios, ranked by urgency |
| Modification suggestions | Recommended Revisions | MUST_FIX / RECOMMENDED / OPTIONAL prioritization |
| Objective evaluation | Balanced Assessment | Overall quality rating, strengths, attack resilience, overreaction warnings |
...
4. Final merge produces the complete report

```
08-report/
├── 01-attack-path-analysis.md
├── 02-misrepresentation-risk.md
├── 03-victim-identification.md
├── 04-interest-damage-map.md
├── 05-contingency-matrix.md
├── 06-recommended-revisions.md
└── 07-balanced-assessment.md

最终报告.md (Final Report)    ← Merged: summary + links to sections
final-report.json              ← Structured data
```
...
└── 20260604-175356-topic-name/
    ...
    │
    └── 08-report/                       # Section-by-section reports
        ├── 01-attack-path-analysis.md
        ├── 02-misrepresentation-risk.md
        ├── 03-victim-identification.md
        ├── 04-interest-damage-map.md
...
This project is a [Claude Code](https://claude.com/claude-code) skill. The core skill definition is in the `.claude/skills/opinion-jury/` directory:
...
> **Verdict: Human review and revision required before publication**
>
> The article identified a real and valuable linguistic phenomenon (some feminine-radical characters underwent semantic degradation), but used fabricated citations from *Shuowen Jiezi*, selective evidence, and universal claims to argue it — methodological failures that are structural, not incidental. **Top risk**: the fabricated "妖，妍也" citation — independently discovered in all 4 panels, zero successful defense. **Top strength**: the core observation survived 144 rounds of cross-examination across 4 panels with 24 participants — real experiences of discrimination for having feminine-radical names were never questioned.
>
> **Highlight**: A rural grandmother pointed out from memory that positive feminine-radical characters (好、安、娟、婷 etc.) far outnumber negative ones — this simple observation was independently cited as an anti-distortion anchor across multiple panels.
...
|----------|-------------|
| Title review | Check if a title might trigger controversy before publishing |
| Post/article review | Review social media posts, blog articles |
| Statement/announcement review | Review brand statements, policy announcements |
| Response review | Review drafted public responses |
| Policy review | Review public policy documents |
| Campaign review | Review marketing copy for potential negative reactions |
